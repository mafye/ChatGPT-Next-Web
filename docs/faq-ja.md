...
nginx のリバースプロキシを使っている場合、設定ファイルに以下のコードを追加する必要がありま���:
...
プロジェクトでは環境変数 CODE でアクセスパスワードを設定していま���。初めて使うときは、設定ページでアクセスコードを入力する必要があります。
...
OpenAI では、指定され��地域のクレジットカードのみご利用いただけます（中国のクレジットカードはご利用いただけません）。お住まいの地域のクレジットカードに対応���ていない場合は、以下の方法があります:
